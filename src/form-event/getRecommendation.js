const users = [
  ...
  const events = [
    ...
    {id: 1, title: 'Обсуждение кино', dateStart: new Date(2020,1,17,9), dateEnd: new Date(2020,1,17,9,30), roomId: 'rid1'},
    {id: 1, title: 'Выбираем алкоголь на корпоратив', dateStart: new Date(2020,1,17,10), dateEnd: new Date(2020,1,17,11), roomId: 'rid1'},
    {id: 1, title: 'Фича-243', dateStart: new Date(2020,1,17,11), dateEnd: new Date(2020,1,17,12), roomId: 'rid1'}
  ]
  ...
  function getRecommendation(members, db) {
    ...
    const matchedRoomsByCapacity =  db.rooms
      .filter(r => r.capacity >= membersCount)
    console.log('matchedRoomsByCapacity', matchedRoomsByCapacity)
    
    //получаем все встречи, проводимые в указанной комнате
    const getEventsInRoom = roomId => db.events.filter(e => e.roomId === roomId)
    ...
    const getEventsInRoomOnDate = (eventsInRoom, date) => eventsInRoom.filter(e => e.dateStart >= date)
    const eventsInRoomOnDay = getEventsInRoomOnDate(eventsInRoom, new Date(2020, 1, 17))
    ...
    const isFreeTimeSpan = (eventsInRoomOnDay, dateStart, dateEnd) => eventsInRoomOnDay
    .every(e => e.dateStart <= dateStart && e.dateEnd <= dateStart || e.dateStart >= dateEnd)
    console.log("isFreeTimeSpan", isFreeTimeSpan(eventsInRoomOnDay, new Date(2020, 1, 17, 9, 30), new Date(2020, 1, 17, 10)))
    
    const getEventsForPeriodOfTime = (eventsOnDay, dateStart, dateEnd) => {

    }
    //этажи участников встречи
    const membersFloors = members.map(uid => users.find(u => u.id === uid).homeFloor)
    ...
    return matchedRoomsByCapacity.sort(
      (r1, r2) => getFloorRemotenessIdx(r1.floor, membersFloors) - getFloorRemotenessIdx(r2.floor, membersFloors)
    )
  }
  
const getRoomsByCapacity = (rooms, capacity = 0) => rooms
    .filter(r => r.capacity >= capacity)

const getEventsForPeriodOfTime = (events, dateStart, dateEnd) => {
    return events.filter(e => {
        return (e.dateStart <= dateStart && e.dateEnd > dateStart)
